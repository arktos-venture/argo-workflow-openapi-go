/*
Argo Server API

You can get examples of requests and responses by using the CLI with `--gloglevel=9`, e.g. `argo list --gloglevel=9`

API version: VERSION
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// StreamResultOfSensorLogEntry struct for StreamResultOfSensorLogEntry
type StreamResultOfSensorLogEntry struct {
	Error *GrpcGatewayRuntimeStreamError `json:"error,omitempty"`
	Result *SensorLogEntry `json:"result,omitempty"`
}

// NewStreamResultOfSensorLogEntry instantiates a new StreamResultOfSensorLogEntry object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStreamResultOfSensorLogEntry() *StreamResultOfSensorLogEntry {
	this := StreamResultOfSensorLogEntry{}
	return &this
}

// NewStreamResultOfSensorLogEntryWithDefaults instantiates a new StreamResultOfSensorLogEntry object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStreamResultOfSensorLogEntryWithDefaults() *StreamResultOfSensorLogEntry {
	this := StreamResultOfSensorLogEntry{}
	return &this
}

// GetError returns the Error field value if set, zero value otherwise.
func (o *StreamResultOfSensorLogEntry) GetError() GrpcGatewayRuntimeStreamError {
	if o == nil || o.Error == nil {
		var ret GrpcGatewayRuntimeStreamError
		return ret
	}
	return *o.Error
}

// GetErrorOk returns a tuple with the Error field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StreamResultOfSensorLogEntry) GetErrorOk() (*GrpcGatewayRuntimeStreamError, bool) {
	if o == nil || o.Error == nil {
		return nil, false
	}
	return o.Error, true
}

// HasError returns a boolean if a field has been set.
func (o *StreamResultOfSensorLogEntry) HasError() bool {
	if o != nil && o.Error != nil {
		return true
	}

	return false
}

// SetError gets a reference to the given GrpcGatewayRuntimeStreamError and assigns it to the Error field.
func (o *StreamResultOfSensorLogEntry) SetError(v GrpcGatewayRuntimeStreamError) {
	o.Error = &v
}

// GetResult returns the Result field value if set, zero value otherwise.
func (o *StreamResultOfSensorLogEntry) GetResult() SensorLogEntry {
	if o == nil || o.Result == nil {
		var ret SensorLogEntry
		return ret
	}
	return *o.Result
}

// GetResultOk returns a tuple with the Result field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StreamResultOfSensorLogEntry) GetResultOk() (*SensorLogEntry, bool) {
	if o == nil || o.Result == nil {
		return nil, false
	}
	return o.Result, true
}

// HasResult returns a boolean if a field has been set.
func (o *StreamResultOfSensorLogEntry) HasResult() bool {
	if o != nil && o.Result != nil {
		return true
	}

	return false
}

// SetResult gets a reference to the given SensorLogEntry and assigns it to the Result field.
func (o *StreamResultOfSensorLogEntry) SetResult(v SensorLogEntry) {
	o.Result = &v
}

func (o StreamResultOfSensorLogEntry) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Error != nil {
		toSerialize["error"] = o.Error
	}
	if o.Result != nil {
		toSerialize["result"] = o.Result
	}
	return json.Marshal(toSerialize)
}

type NullableStreamResultOfSensorLogEntry struct {
	value *StreamResultOfSensorLogEntry
	isSet bool
}

func (v NullableStreamResultOfSensorLogEntry) Get() *StreamResultOfSensorLogEntry {
	return v.value
}

func (v *NullableStreamResultOfSensorLogEntry) Set(val *StreamResultOfSensorLogEntry) {
	v.value = val
	v.isSet = true
}

func (v NullableStreamResultOfSensorLogEntry) IsSet() bool {
	return v.isSet
}

func (v *NullableStreamResultOfSensorLogEntry) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStreamResultOfSensorLogEntry(val *StreamResultOfSensorLogEntry) *NullableStreamResultOfSensorLogEntry {
	return &NullableStreamResultOfSensorLogEntry{value: val, isSet: true}
}

func (v NullableStreamResultOfSensorLogEntry) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStreamResultOfSensorLogEntry) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


